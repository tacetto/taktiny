...
class Encoder(nn.Module):
    def __init__(
        ...
        for i, dim in enumerate(dims):
            ...
            for _ in range(layers_per_block):
                self.down_blocks.layers.append(
                    modules.ResnetBlock2D(in_channels=current_channels, out_channels=dim, seed=seed)
                )
        ...
        # Mid Block
        self.mid_block1 = modules.ResnetBlock2D(in_channels=current_channels, out_channels=current_channels, seed=seed)
        self.mid_attn = SpatialAttention(dim=current_channels, seed=seed)
        self.mid_block2 = modules.ResnetBlock2D(in_channels=current_channels, out_channels=current_channels, seed=seed)
